        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                $.ajax({
                            dataType:'json',
                            async:false,
                            url:"../../../../../qiniu/getToken?key="+filename,
                            success:function(d) {
                                token = d.uptoken;
                                key=d.key;
                            }
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (!ret.error) {
                    	json={'state':'SUCCESS',
                    		'original':file.name,
                    		'url':editor.getOpt('qiniuHost')+ret.key
                    	};
                        _this.imageList.push(json);
                        $file.append('<span class="success"></span>');
                    } else {
                    	json={'state':ret.error};
    ...
    OnlineImage.prototype = {
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                var url=editor.options.imageManagerUrl;
                var isJsonp=false;
                ...
                ajax.request(url, {
                    ...
                    'onsuccess': function (r) {
                        try {
                        	var list = isJsonp ? r:eval('(' + r.responseText + ')');
                        	var json={"list":[],"total":list.length,"start":1};
                        	for(var i=0;i<list.length;i++){
                        		json.list.push({
                        			"mtime":list[i].putTime,
                        			"url":editor.getOpt('qiniuHost')+list[i].key
                        		});
                        	}
