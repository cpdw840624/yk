...
<html>
	<head>
		...
		<meta charset="UTF-8">
		<title>红黄蓝基础信息平台 - 登录</title>

		<link rel="stylesheet"
			href="<%=cachePath %>/res/css/normalize.css">
...
body {
	...
	filter: progid : DXImageTransform.Microsoft.gradient (   startColorstr =
		'#3E1D6D', endColorstr = '#092756', GradientType = 1 );
}
...
var sync_token_from_session = function (asyncTokenId) {
	...
	<body ng-app="myApp" ng-controller="myCtrl">
		<div class="login">
			<h3>红黄蓝基础信息平台</h3>
			<form method="post">
				<input type="text" autofocus="autofocus" ng-model="username" placeholder="企业邮箱账号" required="required" style="width:185px;"/><span style="color:white;" ng-bind="mailServer"></span>
				<input type="password" ng-model="password" placeholder="企业邮箱密码" required="required">
				<button type="button" class="btn btn-primary btn-block btn-large" ng-click="login()">
					登录
				</button>
			</form>
		</div>
...
app.controller('myCtrl', function($scope) {
	...
	$scope.mailServer="@rybbaby.com";
	$scope.login=function(){
		var url=contextPath+"/base/login";
			var params=new Array();
			params.push("username="+$scope.username+$scope.mailServer);
			params.push("password="+$scope.password);
		...
		$.ajax({
			...
			success : function(ret){
				if(ret){
					if(ret.result){
						window.location.href=contextPath+"/base/main";
					}else{
